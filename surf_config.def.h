...
static char *useragent      = "Mozilla/5.0 (X11; U; Unix; en-US) "
                              ...
                              "Chrome/47.0.2526.106 Safari/537.15 Surf/"VERSION;
...
#define DOWNLOAD(d, r) { \
	.v = (char *[]){ "/bin/sh", "-c", \
	     "xterm -title \"Download monitor\" " \
             "-e \"cd ${HOME}/Downloads; ftp -c '$2' -U '$1' '$0';" \
	     "echo 'Hit a key to close this window'; read;\"", \
...
/* LOAD_BOOKMARK(PROP) */
#define LOAD_BOOKMARK(p) { \
        .v = (char *[]){ "/bin/sh", "-c", \
             "xprop -id $0 -f $1 8s -set $1 `load_bookmark` || exit 0", \
             winid, p, NULL } \
}
...
#define GO_HOME(p) { \
        .v = (char *[]){ "/bin/sh", "-c", \
             "xprop -id $0 -f $1 8s -set $1 `cat ${HOME}/.surf/homepage` || exit 0", \
             winid, p, NULL } \
}
...
#define NETSEARCH(p) { \
        .v = (char *[]){ "/bin/sh", "-c", \
             "xprop -id $0 -f $1 8s -set $1 `netsearch` || exit 0", \
             winid, p, NULL } \
}
...
static Key keys[] = {
	...
	{ MODKEY|GDK_SHIFT_MASK,GDK_g,      togglegeolocation, { 0 } },
        ...
        { MODKEY,               GDK_d,      spawn,      ADD_BOOKMARK("_SURF_URI") },
        { MODKEY|GDK_SHIFT_MASK,GDK_d,      spawn,      LOAD_BOOKMARK("_SURF_GO") },

        { MODKEY,               GDK_a,      spawn,      GO_HOME("_SURF_GO") },

        { MODKEY,               GDK_s,      spawn,      NETSEARCH("_SURF_GO") },
};
